...
export class RegisterComponent implements OnInit {
  ...
  register() {
    ...
    if (!this.createdRoleUser) {
      if (this.user.edu == null || this.user.edu.trim().length == 0) {alert('Заполните поле "Образование"');return;}
      if (this.user.prof == null || this.user.prof.trim().length == 0) {alert('Заполните поле "Специализация"');return;}
    }
